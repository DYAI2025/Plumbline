...
**Round-trip ID continuity test for swap targets (learned):** When an HTMX/AJAX swap depends on consistent IDs across two artifacts — the base page declares `hx-target="#X"` and the partial returns a wrapper element with `id="X"` — testing each direction independently misses divergence. Single-direction tests assert (a) "base page contains `hx-target='#X'`" and (b) "partial response contains `<ul id='X'>`" but both pass even when the two literals disagree. Write a round-trip test that EXTRACTS both ids at runtime (`re.search(r'hx-target="#([\w-]+)"', base_body).group(1)` AND `re.match(r'<\w+ id="([\w-]+)"', partial_body).group(1)`) and ASSERTS THEY ARE EQUAL. Without the round-trip pin, the first swap works (HTMX finds `#X`, replaces it), the new wrapper has `id="Y"`, subsequent swaps fail to locate `#X` and silently break — users see "the dropdown stops updating after the first keystroke" without a single failed test. Apply the same pattern to any cross-artifact identifier contract: template-CSS class names, JSON field names shared between schema and renderer, event-name strings between dispatcher and listener.

**Slice-based assertions need fixture-shift falsification (learned):** When an assertion narrows the response body via a regex window, substring slice, or `body.find(token, start)` pattern, the slice must cover the intended scope under NON-DEFAULT fixture variations. The classic bug shape: `board_start = body.find('id="board"'); board_end = body.find("</div>", board_start)` returns the FIRST `</div>` after `id="board"`, which is at most the first card's inner element close — NOT the board container's close. Tests pass only because the default conftest fixture happens to seed into the first column with no optional markup. Add at least ONE falsification test per slice-based assertion that seeds the fixture in a different state (non-default column, with optional markup present, with extra/missing fields) and runs the SAME assertions. If the slice silently shrinks (or expands) and the assertions become vacuously true under shift, the falsification reveals it. The default-fixture case is necessary but NOT sufficient — slice-based scoping is fundamentally fragile to fixture variation.

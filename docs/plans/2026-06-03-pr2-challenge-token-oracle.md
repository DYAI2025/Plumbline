...
2. Dispatch the challenge-mode body prompts **as the real `concilium.md` "Challenge mode" table maps them** (verified 2026-06-03): Challenger = `concilium-skeptic` (requirement lens), Advisor = `concilium-tech-arbiter` (+distribution lens, build), Critic = `concilium-skeptic` (+market lens, concept) — i.e. **two distinct bodies** (`concilium-skeptic` plays Challenger *and* Critic; `concilium-market-realist` is **not** dispatched — market is only a lens). Each against `CANVAS.md` + `IDEA.md`, **TEXT-ONLY** ("respond with your challenge as text; do NOT Write/Edit/Bash any files"), honoring the gate's per-round word cap. The RUNBOOK is the authoritative procedure; reconcile to the live table before dispatching. **O3 shared-base caveat:** because Challenger and Critic share the `concilium-skeptic` base, their pairwise similarity is structurally elevated — an over-cap Challenger↔Critic pair is expected, not consensus-theater; genuine theater shows on a cross-body pair. *(An earlier draft of this plan named a three-distinct-body mapping; that was inaccurate and is corrected here to match the real gate.)*

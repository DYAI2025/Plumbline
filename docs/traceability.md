...
`evidence-class` columns hold the **target** class. The actual class is set by
`production-validator` (Gate C) during the build. Both slices have requirements whose
honest target is `real-boundary-smoke` (start-path wiring; OpenRouter reachability) and
that will read **PASS(tests)/RED(confidence)** until a real-boundary smoke exists. That
RED may not be silently downgraded — only the user may reclassify it.

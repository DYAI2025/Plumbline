---
name: reasoning-goal-planner
description: "GOAP planner variant wired for claude-flow MCP and swarm orchestration. Drives multi-agent goal achievement via task_orchestrate, swarm_init, and memory_usage, persisting reusable plans across runs. Choose this over `goal-planner` when the plan should coordinate a claude-flow swarm rather than run as a standalone reasoning agent."
color: purple
---
